...
#define XMDIST	(TinselAboveV1 ? 6 : 4)
#define XHMDIST	(TinselAboveV1 ? 3 : 2)
#define YMDIST	(TinselAboveV1 ? 3 : 2)
#define YHMDIST	(TinselAboveV1 ? 3 : 2)
...
#define STEPS_MAX (TinselAboveV1 ? 12 : 6)
...
static void GotThere(MOVER *pMover) {
	...
	if (TinselAboveV1 && pMover->bIgPath && pMover->zOverride != -1
...
static void SetMoverIntDest(MOVER *pMover, int x, int y) {
	...
	if (pMover->hCpath == hTpath || pMover->bIgPath
		...
		pMover->hIpath = !TinselAboveV1 ? hTpath : InPolygon(x, y, PATH);
		...
		if (TinselAboveV1 && (hIpath == NOPOLY)) {
...
static void EnteringNewPath(MOVER *pMover, HPOLYGON hPath, int x, int y) {
	...
	if (PolySubtype(hPath) == NODE) {
		...
		// Added 23/10/96
		if (TinselAboveV1 && (pMover->hRpath == hPath))
			return;
