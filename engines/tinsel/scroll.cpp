...
#define RLDISTANCE (TinselAboveV1 ? _scrollData.xTrigger : 50)
#define UDISTANCE (TinselAboveV1 ? _scrollData.yTriggerTop : 20)
#define DDISTANCE (TinselAboveV1 ? _scrollData.yTriggerBottom : 20)
...
void Scroll::ScrollImage() {
	...
	if (_leftScroll > 0) {
		...
		if (TinselAboveV1 && SysVar(SV_MaximumXoffset) &&  (Loffset > SysVar(SV_MaximumXoffset)))
		...
		/*** New feature to prop up rickety scroll boundaries ***/
		if (TinselAboveV1 && SysVar(SV_MinimumXoffset) &&  (Loffset < SysVar(SV_MinimumXoffset)))
	...
	if (_downScroll > 0) {
		...
		/*** New feature to prop up rickety scroll boundaries ***/
		if (TinselAboveV1 && SysVar(SV_MaximumYoffset) &&  Toffset > SysVar(SV_MaximumYoffset))
		...
		/*** New feature to prop up rickety scroll boundaries ***/
		if (TinselAboveV1 && SysVar(SV_MinimumYoffset) &&  Toffset < SysVar(SV_MinimumYoffset))
...
void Scroll::ScrollTo(int x, int y, int xIter, int yIter) {
	...
	_scrollPixelsX = xIter != 0 ? xIter : (TinselAboveV1 ? _scrollData.xSpeed : SCROLLPIXELS);
	_scrollPixelsY = yIter != 0 ? yIter : (TinselAboveV1 ? _scrollData.ySpeed : SCROLLPIXELS);
