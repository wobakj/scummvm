...
void Poly::nextPoly() {
	...
	if ((FROM_32(typeVal) == 6) && TinselV3)
		typeVal = TO_32(7);
	if ((FROM_32(typeVal) == 5) && TinselAboveV1)
		typeVal = TO_32(6);
...
void GetTagTag(HPOLYGON hp, SCNHANDLE *hTagText, int *tagx, int *tagy) {
	...
	*tagx = (int)FROM_32(ptp.tagx) + (TinselAboveV1 ? volatileStuff[hp].xoff : 0);
	*tagy = (int)FROM_32(ptp.tagy) + (TinselAboveV1 ? volatileStuff[hp].yoff : 0);
...
static void SetPathAdjacencies() {
	...
	for (i1 = 0; i1 < MAX_POLY-1; i1++) {
		...
		for (i2 = i1 + 1; i2 < MAX_POLY; i2++) {
			...
			if (TinselAboveV1 && !MatchingLevels(p1, p2))
				continue;
...
void GetPolyNode(HPOLYGON hp, int *pNodeX, int *pNodeY) {
	...
	if (TinselAboveV1 && (pHandle == 0x74191900) && (hp == 8)) {
		*pNodeX = 480;
		*pNodeY = 408;
	} else {
