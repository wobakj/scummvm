...
void KeyboardProcess(CORO_PARAM, const void *) {
	...
	while (true) {
		...
		case Common::KEYCODE_m:
			...
			if (TinselAboveV1 && (evt.kbd.hasFlags(Common::KBD_ALT)))
 ...
 */
void SetNewScene(SCNHANDLE scene, int entrance, int transition) {
	if (!g_bCuttingScene && TinselAboveV1)
		WrapScene();
...
static void RestoredProcess(CORO_PARAM, const void *param) {
	...
	_ctx->bConverse = TinselAboveV1 && (_ctx->pic->event == CONVERSE);
	...
	case Common::EVENT_MOUSEMOVE:
		{
			...
			int ySkip = TinselAboveV1 ? (g_system->getHeight() - _vm->screen().h) / 2 : 0;
