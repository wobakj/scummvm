...
#define IX_CROSS1 (TinselAboveV1 ? IX2_CROSS1 : IX1_CROSS1)
#define IX_CURDD (TinselAboveV1 ? IX2_CURDD : IX1_CURDD)
#define IX_CURDU (TinselAboveV1 ? IX2_CURDU : IX1_CURDU)
#define IX_CURLR (TinselAboveV1 ? IX2_CURLR : IX1_CURLR)
#define IX_CURUD (TinselAboveV1 ? IX2_CURUD : IX1_CURUD)
#define IX_CURUL (TinselAboveV1 ? IX2_CURUL : IX1_CURUL)
#define IX_MDGROOVE (TinselAboveV1 ? IX2_MDGROOVE : IX1_MDGROOVE)
#define IX_MDSLIDER (TinselAboveV1 ? IX2_MDSLIDER : IX1_MDSLIDER)
#define IX_NTR (TinselAboveV1 ? IX2_NTR : IX1_NTR)
#define IX_RBR (TinselAboveV1 ? IX2_RBR : IX1_RBR)
#define IX_RTL (TinselAboveV1 ? IX2_RTL : IX1_RTL)
#define IX_RTR (TinselAboveV1 ? IX2_RTR : IX1_RTR)
#define IX_TICK1 (TinselAboveV1 ? IX2_TICK1 : IX1_TICK1)
...
#define MAX_ININV (TinselAboveV1 ? 160 : 150) // Max in an inventory

#define ITEM_WIDTH (TinselAboveV1 ? 50 : 25)  // Dimensions of an icon
#define ITEM_HEIGHT (TinselAboveV1 ? 50 : 25) //
#define I_SEPARATION (TinselAboveV1 ? 2 : 1)  // Item separation

#define NM_TOFF 11                // Title text Y offset from top
#define NM_TBT (TinselAboveV1 ? 4 : 0) // Y, title box top
#define NM_TBB 33
#define NM_LSX (TinselAboveV1 ? 4 : 0) // X, left side
#define NM_BSY (TinselAboveV1 ? -9 : -M_TH + 1)
#define NM_RSX (TinselAboveV1 ? -9 : -M_SW + 1)
#define NM_SBL (-27)
#define NM_SLH (TinselAboveV1 ? 11 : 5) // Slider height
#define NM_SLX (-11)               // Slider X offset (from right)

#define NM_BG_POS_X (TinselAboveV1 ? 9 : 1)    // }
#define NM_BG_POS_Y (TinselAboveV1 ? 9 : 1)    // } Offset of translucent rectangle
#define NM_BG_SIZ_X (TinselAboveV1 ? -18 : -3) // }
#define NM_BG_SIZ_Y (TinselAboveV1 ? -18 : -3) // } How much larger it is than edges
...
#define NM_SLIDE_INSET (TinselAboveV1 ? 18 : 9)     // X offset (from right) of left of scroll region
#define NM_SLIDE_THICKNESS (TinselAboveV1 ? 13 : 7) // thickness of scroll region
...
#define MD_YBUTTOP (TinselAboveV1 ? 2 : 9)
#define MD_YBUTBOT (TinselAboveV1 ? 16 : 0)
#define MD_XLBUTL (TinselAboveV1 ? 4 : 1)
#define MD_XLBUTR (TinselAboveV1 ? 26 : 10)
#define MD_XRBUTL (TinselAboveV1 ? 173 : 105)
#define MD_XRBUTR (TinselAboveV1 ? 195 : 114)
#define ROTX1 60 // Rotate button's offsets from the center

#define MAX_NAME_RIGHT (TinselAboveV1 ? 417 : 213)

#define SLIDE_RANGE (TinselAboveV1 ? 120 : 81)
#define SLIDE_MINX (TinselAboveV1 ? 25 : 8)
#define SLIDE_MAXX (TinselAboveV1 ? 25 + 120 : 8 + 81)

#define MDTEXT_YOFF (TinselAboveV1 ? -1 : 6)
#define MDTEXT_XOFF -4
#define TOG2_YOFF -22
#define ROT_YOFF 48
#define TYOFF (TinselAboveV1 ? 4 : 0)
#define FLAGX (-5)
#define FLAGY 4
...
#define ciOption (TinselAboveV1 ? t2ciOption : t1ciOption)
#define optionBox (TinselAboveV1 ? t2OptionBox : t1OptionBox)
...
#define ciLoad (TinselAboveV1 ? t2ciLoad : t1ciLoad)
#define loadBox (TinselAboveV1 ? t2LoadBox : t1LoadBox)
#define ciSave (TinselAboveV1 ? t2ciSave : t1ciSave)
#define saveBox (TinselAboveV1 ? t2SaveBox : t1SaveBox)
...
#define ciRestart (TinselAboveV1 ? t2ciRestart : (TinselV1PSX ? t1ciRestartPSX : t1ciRestart))
...
#define ciSound (TinselAboveV1 ? t2ciSound : t1ciSound)
...
#define quitBox (TinselAboveV1 ? t2QuitBox : t1QuitBox)
#define ciQuit (TinselAboveV1 ? t2ciQuit : t1ciQuit)
...
#define START_ICONX (TinselAboveV1 ? 12 : (M_SW + 1))         // } Relative offset of first icon
#define START_ICONY (TinselAboveV1 ? 40 : (M_TBB + M_TH + 1)) // } within the inventory window
...
void Dialogs::Select(int i, bool force) {
	...
	case RGROUP:
		_iconArray[HL2] = RectangleObject(_vm->_bg->BgPal(),
		                                   (TinselAboveV1 ? HighlightColor() : COL_HILIGHT), cd.box[i].w, cd.box[i].h);
...
void Dialogs::AddToInventory(int invno, int icon, bool hold) {
	...
	assert(invno == INV_1 || invno == INV_2 || invno == INV_CONV || invno == INV_OPEN || (invno == INV_DEFAULT && TinselAboveV1));
	...
	if (invno == INV_OPEN) {
		...
		if (TinselAboveV1 && invno == INV_DEFAULT) {
...
bool Dialogs::RemFromInventory(int invno, int icon) {
	...
	if (i == _invD[invno].NoofItems)
		...
		if (TinselAboveV1 && invno == INV_CONV) {
...
void Dialogs::HoldItem(int item, bool bKeepFilm) {
	...
	if (_heldItem != item) {
		if (TinselAboveV1 && (_heldItem != NOOBJECT)) {
...
int Dialogs::WhichMenuBox(int curX, int curY, bool bSlides) {
	...
	if (cd.bExtraWin) {
		const Common::Rect r = TinselAboveV1 ? Common::Rect(411, 46, 425, 339) : Common::Rect(20 + 181, 24 + 2, 20 + 181 + 8, 24 + 139 + 5);

		if (r.contains(curX, curY)) {

			if (curY < (r.top + (TinselAboveV1 ? 18 : 5)))
				return IB_UP;
			else if (curY > (r.bottom - (TinselAboveV1 ? 18 : 5)))
				return IB_DOWN;
	...
	if (index < 0) {
		...
		if ((cd.box[cd.pointBox].boxType == ARSBUT && cd.selBox != NOBOX) ||
			...
			_iconArray[HL1] = RectangleObject(_vm->_bg->BgPal(),
			                                   (TinselAboveV1 ? HighlightColor() : COL_HILIGHT),
...
void Dialogs::AddSlider(OBJECT **slide, const FILM *pfilm) {
	...
	MultiSetAniXY(*slide, MultiRightmost(_rectObject) + (TinselAboveV1 ? NM_SLX : -M_SXOFF + 2),
...
void Dialogs::AddBox(int *pi, const int i) {
	...
	default:
		...
		if (TinselAboveV1 && (cd.box[i].textMethod == TM_NONE))
			break;

		// Give us a box
		_iconArray[*pi] = RectangleObject(_vm->_bg->BgPal(), TinselAboveV1 ? BoxColor() : COL_BOX,
		...
		*pi += 1;
		...
		if ((cd.box[i].textMethod == TM_POINTER) ||
		    (!TinselAboveV1 && (cd.box[i].ixText == USE_POINTER))) {
			if (cd.box[i].boxText != NULL) {
			...
			if (TinselAboveV1 && (cd.box[i].boxType == RGROUP))
...
int Dialogs::AddExtraWindow(int x, int y, OBJECT **retObj) {
	...
	x += TinselAboveV1 ? 30 : 20;
	y += TinselAboveV1 ? 38 : 24;
	...
	MultiSetAniXY(retObj[n], x + (TinselAboveV1 ? _TLwidth + 312 : 152), y);
	MultiSetZPosition(retObj[n], Z_INV_MFRAME);
	n++;
	retObj[n] = AddObject(&pfilm->reels[IX_BL], -1); // Bottom left
	MultiSetAniXY(retObj[n], x, y + (TinselAboveV1 ? _TLheight + 208 : 124));
	MultiSetZPosition(retObj[n], Z_INV_MFRAME);
	n++;
	retObj[n] = AddObject(&pfilm->reels[IX_BR], -1); // Bottom right
	MultiSetAniXY(retObj[n], x + (TinselAboveV1 ? _TLwidth + 312 : 152),
	              y + (TinselAboveV1 ? _TLheight + 208 : 124));
	MultiSetZPosition(retObj[n], Z_INV_MFRAME);
	n++;

	// Draw the edges
	retObj[n] = AddObject(&pfilm->reels[IX_H156], -1); // Top
	MultiSetAniXY(retObj[n], x + (TinselAboveV1 ? _TLwidth : 6), y + NM_TBT);
	MultiSetZPosition(retObj[n], Z_INV_MFRAME);
	n++;
	retObj[n] = AddObject(&pfilm->reels[IX_H156], -1); // Bottom
	MultiSetAniXY(retObj[n], x + (TinselAboveV1 ? _TLwidth : 6), y + (TinselAboveV1 ? _TLheight + 208 + _BLheight + NM_BSY : 143));
	MultiSetZPosition(retObj[n], Z_INV_MFRAME);
	n++;
	retObj[n] = AddObject(&pfilm->reels[IX_V104], -1); // Left
	MultiSetAniXY(retObj[n], x + NM_LSX, y + (TinselAboveV1 ? _TLheight : 20));
	MultiSetZPosition(retObj[n], Z_INV_MFRAME);
	n++;
	retObj[n] = AddObject(&pfilm->reels[IX_V104], -1); // Right 1
	MultiSetAniXY(retObj[n], x + (TinselAboveV1 ? _TLwidth + 312 + _TRwidth + NM_RSX : 179),
	              y + (TinselAboveV1 ? _TLheight : 20));
	MultiSetZPosition(retObj[n], Z_INV_MFRAME);
	n++;
	retObj[n] = AddObject(&pfilm->reels[IX_V104], -1); // Right 2
	MultiSetAniXY(retObj[n], x + (TinselAboveV1 ? _TLwidth + 312 + _TRwidth + NM_SBL : 188),
	              y + (TinselAboveV1 ? _TLheight : 20));
	MultiSetZPosition(retObj[n], Z_INV_MFRAME);
	n++;
...
void Dialogs::ConstructInventory(InventoryType filling) {
	...
	if (TinselAboveV1 && (_activeInv == INV_CONV)) {
		_TL = IX_TL;
		_TR = IX2_TR4;
		_BL = IX_BL;
		_BR = IX_RBR;
	...
	if (_SuppH) {
		int offx = _TLwidth + eH - (TinselAboveV1 ? ITEM_WIDTH + I_SEPARATION : 26);
	...
	if (_SuppV) {
		int offy = _TLheight + eV - (TinselAboveV1 ? ITEM_HEIGHT + I_SEPARATION : 26);
		int minAmount = TinselAboveV1 ? 20 : 5;
		if (offy < minAmount)
			offy = minAmount;
	...
	if (TinselAboveV1 && (filling != EMPTY)) {
	...
	if (filling == FULL) {
		...
		if (_activeInv == INV_CONV) {
			...
			_sliderYmin = _TLheight - (TinselAboveV1 ? 1 : 2);
			_sliderYmax = _TLheight + eV + (TinselAboveV1 ? 12 : 10);
...
void Dialogs::ConvAction(int index) {
	...
	MOVER *pMover = TinselAboveV1 ? GetMover(_vm->_actor->GetLeadId()) : NULL;
...
void Dialogs::HideConversation(bool bHide) {
	...
	if (_inventoryState == ACTIVE_INV && _activeInv == INV_CONV) {
		if (bHide) {
			...
			if (TinselAboveV1 && _ItemsChanged)
			...
			if (TinselAboveV1 && _bMoveOnUnHide) {
				/*
...
void Dialogs::InvPickup(int index) {
	...
	if (_heldItem == INV_NOICON && _invD[_activeInv].contents[index] &&
	    (!TinselAboveV1 || _invD[_activeInv].contents[index] != _heldItem)) {
		// Pick-up
		...
		if (_activeInv == INV_CONF) {
			...
			if (index != INV_NOICON) {
				if (_invD[_activeInv].contents[index] && _invD[_activeInv].contents[index] != _heldItem) {
					...
					if (TinselAboveV1)
						_thisIcon = _invD[_activeInv].contents[index];
					if (TinselAboveV1 || (invObj->hScript))
...
void Dialogs::setInvWinParts(SCNHANDLE hf) {
	...
	assert(FROM_32(pfilm->numreels) >= (uint32)(TinselAboveV1 ? T2_HOPEDFORREELS : T1_HOPEDFORREELS)); // not as many reels as expected
#endif
}
...
static void InvTinselEvent(INV_OBJECT *pinvo, TINSEL_EVENT event, PLR_EVENT be, int index) {
	...
	if (_vm->_dialogs->InventoryIsHidden() || (TinselAboveV1 && !pinvo->hScript))
		return;
