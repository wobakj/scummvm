...
static bool PolyTag(HotSpotTag *pTag, OBJECT **ppText) {
	...
	for (int i = 0; i < MAX_POLY; i++) {
		...
		if ((hp != NOPOLY) && (PolyPointState(hp) == PS_POINTING) && (PolyTagState(hp) != TAG_ON)) {
			...
			return true;
		} else if ((TinselAboveV1 && PolyTagIsWanted(hp)) ||
			(!TinselAboveV1 && hp != NOPOLY && PolyTagState(hp) == TAG_ON)) {
			...
			if (newPoly) {
				...
				if (strLen == 0)
					...
					ppText = nullptr;
				else if (TinselAboveV1 && !PolyTagFollowsCursor(hp)) {
					// May have buggered cursor
					_vm->_cursor->EndCursorFollowed();
