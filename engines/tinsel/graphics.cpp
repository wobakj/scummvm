...
void UpdateScreenRect(const Common::Rect &pClip) {
	int yOffset = TinselAboveV1 ? (g_system->getHeight() - SCREEN_HEIGHT) / 2 : 0;
...
void DrawObject(DRAWOBJECT *pObj) {
	...
	int packType = pObj->flags >> 14;	// TinselAboveV1

	if (TinselAboveV1 && packType != 0) {
		// Color packing for TinselAboveV1
		...
		case 0x02:	// TinselAboveV1, draw sprite without clipping
		case 0x11:	// TinselAboveV1, draw sprite without clipping, flipped horizontally
		case 0x42:	// TinselAboveV1, draw sprite with clipping
		case 0x51:	// TinselAboveV1, draw sprite with clipping, flipped horizontally
			assert(TinselAboveV1 || (typeId == 0x01 || typeId == 0x41));
		...
		case 0x48:	// draw background with clipping
			...
			else if (TinselAboveV1 || TinselV1Mac || TinselV0)
